...
export class AuthService {
  ...
  async Signup(fullname: string, email: string, phone: string, password: string, role?: string): Promise<any> {
    ...
    const token = await this.jwtService.signAsync({ id: newUser._id, fullname: newUser.fullname, email: newUser.email, role: newUser.role });
  ...
  async changePassword(email: string, oldpass: string, newpass: string): Promise<any> {
    ...
    const comparedPassword = await bcrypt.compare(oldpass, existingUser.password);
    if (!comparedPassword) {
      throw new HttpException(
